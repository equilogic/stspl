...
#        else:
        ...
        sql = """
            ...
            m.name AS move_name, m.id AS mmove_id,per.code as period_code, m.transaction_key AS transaction_code,
        ...
        if res_lines and context.get('datas').get('init_balance'):
            ...
            sql = """
                ...
                '' AS move_name, '' AS mmove_id, '' AS period_code, '' AS transaction_code,
                '' AS currency_code,
                NULL AS currency_id,
    ...
    def _generate_file(self, cr, uid, context = None):
        ...
        try:
                ...
                for line in supplier.invoice_line:
                    ...
                    for tax in line.invoice_line_tax_id:
                        ...
                        if tax_data:
                            ...
                            tax_rec = tax_code_obj.browse(cr, uid, tax_data[0].get('tax_code_id'))
                            if tax_rec.code:
                                tax_name = tax_rec and tax_rec.code
                        if supplier.currency_id.id == supplier.company_id.currency_id.id:
                            GSTValueSGD = tax_amt
                            TaxCode += tax_name
                            GSTFCY = 0.0
            ...
            for acc in child_data:
                ...
                for ac in acc_data:
                    ...
                    move_data = tools.ustr(datetime.datetime.strptime(ac.get('ldate'), DEFAULT_SERVER_DATE_FORMAT).strftime('%d/%m/%Y') or '') + \
                                ...
                                '|'.ljust(1) + \
                                tools.ustr(ac.get('transaction_code') or '') + \
                                '|'.ljust(1) + \
            ...
            account_glend_data = tools.ustr('GLDataEnd|') + \
                        tools.ustr(float(tot_debit) or 0.0) + \
                        '|'.ljust(1) + \
                        tools.ustr(float(tot_credit) or 0.0) + \
                        '|'.ljust(1) + \
