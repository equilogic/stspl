...
#                    sum_pr = code.sum_period
    ...
    def get_info(self, form, context=None):
        ...
        account_ids = account_obj.search(cr, uid, [('name', '=','Revenue')], context=ctx)
        period_list = []
        ...
        box8 = box6 - box7
        if account_ids:
            account_rec = account_obj.browse(cr, uid, account_ids[0], context=ctx)
            box13 = account_rec.balance
        tax_dics.update({
            'name': company_name,
            'tax_no': tax_no,
            'gst_no': gst_no,
...
report_sxw.report_sxw('report.gstreturnf7.odoo.new', 'account.tax.code',
    'addons/sg_account_odoo/report/gst_returnf7_report.rml', parser=report_gst_returnf7, header=False)
